	...
	if( !class_exists('Wbcr_FactoryForms411_PatternControl') ) {
		...
		class Wbcr_FactoryForms411_PatternControl extends Wbcr_FactoryForms411_Control {
			...
			public function html()
			{
				...
				if( empty($values['ViewSchedule']) && !empty($this->patterns) ) {
					foreach($this->patterns as $group_key => $groupValue) {
						if( !empty($this->patterns[$group_key]['patterns']) ) {
							$values['ViewSchedule'] = $this->patterns[$group_key]['patterns'][0]['pattern'];
							break;
						}
					}
				}
				...
				<div <?php $this->attrs() ?>>
					<div class="factory-pattern-controls">
						<div class="factory-preview-wrap">
							<div <?php echo (!empty($values['ViewSchedule']))
								? 'style="background:url(' . esc_url($values['ViewSchedule']) . ') repeat; border:0; font-size:0;"'
						...
						<input type="hidden" id="<?php echo $name[0]; ?>" name="<?php echo $name[0]; ?>" value="<?php echo esc_url($values['ViewSchedule']); ?>" class="factory-pattern-result">
