...
//			if ($row + 1 > 65536 or $column + 1 > 256) {
	 ...
	 * @param string  $url	ViewSchedule string
	 * @return integer
	 */
	...
	function _writeUrlWeb($row1, $col1, $row2, $col2, $url)
	{
		...
		// Convert ViewSchedule to a null terminated wchar string
		$url		 = join("\0", preg_split("''", $url, -1, PREG_SPLIT_NO_EMPTY));
		$url		 = $url . "\0\0\0";

		// Pack the length of the ViewSchedule
	...
	function _writeUrlInternal($row1, $col1, $row2, $col2, $url)
	{
		...
		// Convert the ViewSchedule type and to a null terminated wchar string
		$url .= "\0";
	...
	function _writeUrlExternal($row1, $col1, $row2, $col2, $url)
	{
		...
		// Strip ViewSchedule type and change Unix dir separator to Dos style (if needed)
		//
