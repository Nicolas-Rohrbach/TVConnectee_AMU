...
class Planning {
    ...
    public function getYaml($file) {
        if (!file_exists(ROOT . '/controllers/fileSchedule/data/' . $file . '.yaml')) {
        ...
        if (function_exists('yaml_parse_file')) {
            return yaml_parse_file(ROOT . '/controllers/fileSchedule/data/' . $file . '.yaml');
        }

        // Symfony YAML
        if (!class_exists('Symfony\Component\Yaml\Parser')) {
            require_once ROOT . '/controllers/fileSchedule/vendor/autoload.php';
        }
        ...
        try {
            return $this->yaml->parse(file_get_contents(ROOT . '/controllers/fileSchedule/data/' . $file . '.yaml'));
    ...
    public function getIdentifier() {
        if (!file_exists(ROOT . '/controllers/fileSchedule/data/identifier')) {
        ...
        $identifiers = file(ROOT . '/controllers/fileSchedule/data/identifier', FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
