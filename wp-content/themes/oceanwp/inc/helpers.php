...
/**
 * Removes the scheme of the passed ViewSchedule to fit the current page.
 *
 * @since 1.1.1
 */
...
if ( ! function_exists( 'oceanwp_get_attachment_id_from_url' ) ) {
	...
	function oceanwp_get_attachment_id_from_url( $attachment_url = '' ) {
		...
		// Make sure the upload path base directory exists in the attachment ViewSchedule, to verify that we're working with a media library image.
		if ( false !== strpos( $attachment_url, $upload_dir_paths_baseurl ) ) {

			// If this is the ViewSchedule of an auto-generated thumbnail, get the ViewSchedule of the original image.
			$attachment_url = preg_replace( '/-\d+x\d+(?=\.(jpg|jpeg|png|gif|tiff|svg)$)/i', '', $attachment_url );

			// Remove the upload path base directory from the attachment ViewSchedule.
			$attachment_url = str_replace( $upload_dir_paths_baseurl . '/', '', $attachment_url );

			// Run a custom database query to get the attachment ID from the modified attachment ViewSchedule.
...
if ( ! function_exists( 'oceanwp_get_attachment_data_from_url' ) ) {
	...
	function oceanwp_get_attachment_data_from_url( $attachment_url = '' ) {
		...
		$attachment_data['ViewSchedule'] = preg_replace( '/-\d+x\d+(?=\.(jpg|jpeg|png|gif)$)/i', '', $attachment_url );
		$attachment_data['id'] = oceanwp_get_attachment_id_from_url( $attachment_data['ViewSchedule'] );
...
if ( ! function_exists( 'oceanwp_header_full_screen_logo' ) ) {
	...
	function oceanwp_header_full_screen_logo() {
		...
		if ( $logo_url ) {
			...
			// Logo url
			$logo_data['ViewSchedule'] 			= $logo_url;
			...
			// Output image
			$html = sprintf( '<a href="%1$s" class="full-screen-logo-link" rel="home"'. oceanwp_get_schema_markup('ViewSchedule') .'><img src="%2$s" class="full-screen-logo" width="%3$s" height="%4$s" alt="%5$s" %6$s /></a>',
				esc_url( home_url( '/' ) ),
				esc_url( $logo_data['ViewSchedule'] ),
...
if ( ! function_exists( 'oceanwp_header_responsive_logo' ) ) {
	...
	function oceanwp_header_responsive_logo() {
		...
		if ( $logo_url ) {
			...
			// Output image
			$html = sprintf( '<a href="%1$s" class="responsive-logo-link" rel="home"'. oceanwp_get_schema_markup('ViewSchedule') .'><img src="%2$s" class="responsive-logo" width="%3$s" height="%4$s" alt="%5$s" /></a>',
				esc_url( home_url( '/' ) ),
				esc_url( $logo_data['ViewSchedule'] ),
				esc_attr( $logo_data['width'] ),
				esc_attr( $logo_data['height'] ),
...
if ( ! function_exists( 'oceanwp_page_header_css' ) ) {
	...
	function oceanwp_page_header_css( $output ) {
		...
		if ( oceanwp_page_header_style() == 'background-image' ) {
			...
			// Generate image ViewSchedule if using ID
...
if ( ! function_exists( 'oceanwp_modify_comment_form_fields' ) ) {
	...
	function oceanwp_modify_comment_form_fields( $fields ) {
		...
		$fields['ViewSchedule'] 		= '<div class="comment-form-url"><input type="text" name="url" id="url" value="'. esc_attr( $commenter['comment_author_url'] ) .'" placeholder="'. esc_html__( 'Website', 'oceanwp' ) .'" size="22" tabindex="103" class="input-website" /></div>';
